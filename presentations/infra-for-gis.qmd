...
# Basemap
bmap <- terrain_map(countries = cntry, terr = terr, 
                    add_labels = TRUE, mask = TRUE)
```
...
Generate a basemap with labels and neighbors countries
...
# Basemap
bmap <- terrain_map(countries = cntry, terr = terr, 
                    add_labels = TRUE, add_neighbors = TRUE, mask = TRUE)
```

Visualize the basemap

```{r}
#| label: ras-basemap-view-3
#| echo: true
#| eval: true
#| warning: false
bmap
...
bmap <- terrain_map(countries = cntry_shp, 
                    adm0 = cntry_shp, adm1 = psnu_shp, terr = terr, 
                    add_labels = TRUE, add_neighbors = TRUE, mask = TRUE)
```
